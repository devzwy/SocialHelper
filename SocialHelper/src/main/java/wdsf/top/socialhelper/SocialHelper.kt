...
import wdsf.top.socialhelper.utils.NetUtil
...
class SocialHelper private constructor() {
    ...
    companion object {
         ...
         * [socialHelperConfig] 可使用[wdsf.top.socialhelper.SocialConfig.Build]进行构造
         */
         ...
         * [onSocialAuthListener] 回调用户授权结果 根据平台不同请传入不同的实体接收 [Platform.WECHAT]时使用[wdsf.top.socialhelper.data.wxapi_resp.WeChatAuth]接收
         */
         ...
         * [onSocialUserInfoListener] 回调用户结果回传 根据平台不同请传入不同的实体接收 [Platform.WECHAT]时使用[wdsf.top.socialhelper.data.wxapi_resp.WeChatUserInfoRespData]接收
         */
